...
plugin は `session.status`、`session.updated`、`session.idle`、`session.error`、`session.compacted`、`todo.updated` と heartbeat を `POST /api/monitor/ingest` に送信します。active 判定は「heartbeat が `OPENCODE_MONITOR_HEARTBEAT_TTL_MS` 以内に到達している source 上の session」です。
