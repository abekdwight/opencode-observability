...
import type { CodexSessionDetailContract } from "../../../src/contracts/codex-sessions";
...
export function CodexSessionDetailPage() {
  ...
  const { data, error, loading } = useJson<CodexSessionDetailContract>(
    `/api/codex-sessions/${encodeURIComponent(id)}`,
  );

  React.useEffect(() => {
    if (data) {
      document.title = `${data.session.title} — Codex Sessions`;
    }
    return () => {
  ...
  const openDetails = React.useMemo(() => new Set<string>(), []);
  const handleToggleDetail = React.useCallback((_id: string) => {}, []);
        ...
        <div className="flex-1 flex flex-col overflow-hidden min-w-0 w-full">
          ...
          >
            <div className="flex gap-[var(--space-sm)] items-center justify-start md:justify-center flex-nowrap overflow-x-auto overscroll-x-contain">
              <button
                type="button"
                className={cn(ctrlBtnBase, collapseEnabled && !plainMode && ctrlBtnActive, plainMode && "opacity-40 cursor-not-allowed")}
                onClick={() => setCollapseEnabled((p) => !p)}
                disabled={plainMode}
                data-testid="btn-collapse"
              >
                <svg width="14" height="14" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="2" role="img" aria-hidden="true">
