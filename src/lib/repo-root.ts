...
export function resolveCanonicalRepoRoot(worktreePath: string): string {
  ...
  if (repoRootCache.has(worktreePath)) return repoRootCache.get(worktreePath) as string;
...
export function resolveRepoBucketKey(worktreePath: string, directoryPath: string): string {
  if (worktreePath === '/') {
